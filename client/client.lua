	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Config.StopFishing) then
				endFishing()
				QBCore.Functions.Notify(translate.FISHING_STOP, 'error')
			end
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z -3.0)  then
					...
					endFishing()
					QBCore.Functions.Notify(translate.FISHING_NOTDEEP, 'error')
				end
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					QBCore.Functions.Notify(translate.FISHING_ENDED, 'error')
				end
			end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					QBCore.Functions.Notify(translate.FISHING_ESCAPED, 'error')
...
CreateThread(function()
	while true do
		...
		if fishing then
			pause = true
			correct = 1
			TriggerEvent('3dme:triggerDisplay', translate.FISHING_TUG)
